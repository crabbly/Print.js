...
describe('addWrapper()', () => {
  it('add a div wrapper to a raw html', () => {
    const params: Partial<PrintJSParams> = {
      // Using Partial as we only need a few properties for this test
      font: 'TimesNewRoman',
      font_size: '12px',
    };
    expect(addWrapper('<span>Test</span>', params as PrintJSParams)).toBe(
      '<div style="font-family:' +
        params.font +
        ' !important; font-size: ' +
        params.font_size +
        ' !important; width:100%;"><span>Test</span></div>'
    );
  });
});
